...
function UpperMenu:init(parent, menu)
    ...
    self._tabs = {
        {name = "world", text = "World", rect = {135, 271, 115, 115}},
        {name = "static", text = "Selection", rect = {256, 262, 115, 115}, enabled = normal},
        {name = "env", text =  "Env", rect = {15, 267, 115, 115}},
        {name = "opt", text = "Options", rect = {385, 385, 115, 115}},
        {name = "save", text = "Save", rect = {260, 385, 115, 115}, callback = callback(self, self, "save"), enabled = normal},
        {name = "move_widget_toggle", text = "<^>", rect = {9, 377, 115, 115}, callback = callback(self, self, "toggle_widget", "move"), enabled = normal and self._parent._has_fix},
        {name = "rotation_widget_toggle", text = "(o)", rect = {137, 383, 115, 115}, callback = callback(self, self, "toggle_widget", "rotation"), enabled = normal and self._parent._has_fix},
    }
    local w = 410
...
function UpperMenu:build_tabs()
    for _, tab in pairs(self._tabs) do
        ...
        local t = self:Tab(tab.name, tab.text, "textures/editor_icons_df", tab.rect, tab.callback, s, tab.enabled)
...
function UpperMenu:Tab(name, text, texture, texture_rect, clbk, s, enabled)
    ...
    if DB.create_entry then
        ...
        return self._menu:Button({
            ...
            w = self._menu:ItemsWidth() / #self._tabs,
            h = self._menu:H(),
        })
    end
end
