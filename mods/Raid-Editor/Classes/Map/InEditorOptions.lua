...
function Options:build_default_menu()
    ...
    if not Global.current_custom_level then
        self:TextBox("MapSavePath", nil, Utils.Path:Combine(BLT.Mods.Constants.maps_directory, Global.game_settings.level_id or ""), {group = main})
    end
    ...
    for _, opt in pairs({"quads", "doors", "blockers", "vis_graph", "coarse_graph", "nav_links", "covers"}) do
        self._draw_options[opt] = self:Toggle(opt, callback(self, self, "draw_nav_segments"), false, {w = w, items_size = 24, offset = 0, group = group})
    end
...
function Options:save()
    ...
    local function save()
        ...
        if Global.current_custom_level then
            local map_dbpath = Path:Combine("levels/mods/", Global.current_custom_level._config.id)
        ...
        for name, data in pairs(worlddef._continent_definitions) do
            local dir = Utils.Path:Combine(map_path, name)
            ...
            missions[name] = {file = Utils.Path:Combine(name, name)}
        end
        ...
        for _, folder in pairs(FileIO:GetFolders(map_path)) do
            if folder ~= "environments" and not worlddef._continent_definitions[folder] then
                FileIO:Delete(Utils.Path:Combine(map_path, folder))
            end
        end
        if bg then
    ...
    if FileIO:Exists(path) then
        local backups_dir = Utils.Path:Combine(BLT.Mods.Constants.maps_directory, "backups")
        FileIO:MakeDir(backups_dir)
        local backup_dir = Utils.Path:Combine(backups_dir, table.remove(string.split(path, "/")))
...
function Options:save_main_xml(include)
    ...
    if data then
        ...
        for i, include_data in ipairs(temp) do
            ...
            if type(include_data) == "table" and include_data.file and FileIO:Exists(Utils.Path:Combine(mod.path, level.include.directory, include_data.file)) then
                local exists
...
function Options:SaveData(path, file_name, data)
    ...
    FileIO:WriteTo(Utils.Path:Combine(path, file_name), data)
end
