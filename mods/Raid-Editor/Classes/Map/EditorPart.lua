...
function Part:init(parent, menu, name, opt, mopt)
    ...
    if not opt.no_title then
        self:Divider("Title", {items_size = 32, offset = 0, background_color = BeardLibEditor.Options:GetValue("AccentColor"), text = string.pretty2(name)})
    end
...
function Part:bind(opt, clbk, in_dialogs)
    ...
    if key then
        local trigger = Utils.Input:TriggerDataFromString(key, clbk)
...
function Part:update()
    local allowed = not BLT.Dialogs:DialogOpened()
    local not_focused = not (self._parent._menu:Focused() or BLT.Dialogs:Menu():Typing())
    for _, trigger in pairs(self._triggers) do
        if not_focused and (allowed or trigger.in_dialogs) and Utils.Input:Triggered(trigger) then
            trigger.clbk()
        end
    end
end
...
function Part:init_basic(parent, name)
    ...
    self._brush:set_font(Idstring(tweak_data.menu.pd2_large_font), 16)
