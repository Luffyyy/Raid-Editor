...
function Editor:init()
    ...
    self._toggle_trigger = Utils.Input:TriggerDataFromString(BeardLibEditor.Options:GetValue("Input/ToggleMapEditor"))
...
function Editor:update(t, dt)
    if Utils.Input:Triggered(self._toggle_trigger) then
...
function Editor:update_camera(t, dt)
    local shft = shift()
    local move = not (self._menu:Focused() or BLT.Dialogs:Menu():Focused())
