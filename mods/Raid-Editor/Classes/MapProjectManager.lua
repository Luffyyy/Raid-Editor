...
function Project:init()
    self._templates_directory = U.Path:Combine(BeardLibEditor.path, "Templates")
    ...
    MenuUtils:new(self)
    self:Divider("This has to be rewritten :(")
    if true then
        return
    end
...
function Project:current_level(data)
    for _, level in pairs(Utils:GetNodeByMeta(data, "level", true) or {}) do
...
function Project:current_mod()
    return Global.current_custom_level and Global.current_custom_level._mod
end

function Project:maps_path()
    return Global.current_custom_level._config.include.directory
end
...
function Project:map_editor_read_xml(file, clean)
    ...
    if clean and data then
        data = Utils:CleanCustomXmlTable(data)
    end
    return data
end
...
function Project:get_clean_data(t, no_clone)
    ...
    for _, v in pairs({"AddFiles", "Hooks"}) do
        Utils:CleanCustomXmlTable(U:GetNodeByMeta(data, v))
    end
    ...
    for _, level in pairs(U:GetNodeByMeta(data, "level", true)) do
        ...
        for _, v in pairs({"include", "assets", "script_data_mods", "add", "hooks", "packages"}) do
            if level and level[v] then
                level[v] = Utils:CleanCustomXmlTable(level[v])
            end
        end
    end
...
function Project:add_existing_level_to_project(data, narr, level_in_chain, narr_pkg, done_clbk)
    self:new_level_dialog(tostring(level_in_chain.level_id), function(name)
        local mod_path = U.Path:Combine(BLT.Mods.Constants.maps_directory, data.name)
        ...
        level.script_data_mods = Utils:CleanCustomXmlTable(deep_clone(self._level_module_template).script_data_mods)
...
function Project:_select_project(mod, save_prev)
    ...
    self:edit_main_xml(self:get_clean_mod_config(mod), function()        
        ...
        local map_path = U.Path:Combine(BLT.Mods.Constants.maps_directory, id)
        local levels = U:GetNodeByMeta(t, "level", true)
        ...
        if t.name ~= id then
            ...
            FileIO:MoveTo(map_path, U.Path:Combine(BLT.Mods.Constants.maps_directory, t.name))
            something_changed = true
        end
...
function Project:delete_level_dialog_clbk(level)
    ...
    if type(level) == "table" then
        FileIO:Delete(U.Path:Combine(BLT.Mods.Constants.maps_directory, t.name, level.include.directory))
...
function Project:create_new_level(name)
    ...
    level.id = name
    local proj_path = U.Path:Combine(BLT.Mods.Constants.maps_directory, t.name)
...
function Project:create_new_narrative(name)
    ...
    data.name = name
    narr.id = name
    local proj_path = U.Path:Combine(BLT.Mods.Constants.maps_directory, name)
