...
function LoadLevelMenu:init()
	...
	local w = self:Toggle("Localized", callback(self, self, "load_levels"), true, opt).w
	w = w + self:Toggle("Vanilla", callback(self, self, "load_levels"), true, opt).w --temp
...
function LoadLevelMenu:load_levels()
    ...
    for id, level in pairs(tweak_data.levels) do
        if type(level) == "table" and (level.world_name and ((level.custom and custom) or (not level.custom and vanilla))) then
...
function LoadLevelMenu:load_level(menu, item)
    ...
    local function load()
        ...
        Global.game_settings.difficulty = "difficulty_2"
